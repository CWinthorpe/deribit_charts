...
try {
    ...
    dbConnect(function (err, db) {
        if (!err) {
            ...
            dbo.collection(config.currency + "_" + config.candle).find({}).sort(theOrder).toArray(function (err, result) {
                if (!err) {
                    //log(result);
                    if (result.length > 0) {
                        log('result not empty');
                        workingCandle = result;
                    } else {
                        log('ticks are empty');
                        workingCandle = [];
                    }
                    log('loaded candle ticks collection');
                } else {
                    log('No tick candles in db');
                    workingCandle = [];
                }
                //db.close();
                wsDB.open('wss://www.deribit.com/ws/api/v2/');
...
var writeCandles = function (candle) {
    dbConnect(function (err, db) {
        if (err) throw err;
        var dbo = db.db("candles");
        dbo.collection(config.currency + "_" + config.candle).insertOne(candle, function (err, res) {
            if (err) throw err;
            //console.log("Candle Inserted");
            //db.close();
        });
    });
}
